...
int blk_find_device(int if_type, int devnum, struct udevice **devp)
{
	...
	{
		//printf("\nret:%d   &uc->dev_head.next:%x     container_of(uc->dev_head.next,struct udevice,uclass_node):%x,自己:%x\n",ret,uc->dev_head.next,container_of(uc->dev_head.next,struct udevice,uclass_node),uc->dev_head);	
	}
